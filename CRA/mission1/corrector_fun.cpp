...
string corrector_fun(WordInputInfo wordInput) {
	...
	if (isInvalidInput(wordInput)) {
		return "";
	}

	vector<WordDbInfo>& dayBest = dayOfWeekBest[WEEK_IDX_CONVERT.at(wordInput.week)];
	...
	double perfectHitMaxPoint = processFullmatchedPoints(wordInput.word, dayBest, weeksBest);
	if (perfectHitMaxPoint > 0) {
		return wordInput.word;
	}

	//찰떡 
	CorrectorProcessResult resultOfCorrector = processCorrector(wordInput.word, dayBest, weeksBest);
	if (resultOfCorrector.isFound) {
		return resultOfCorrector.correctedStr;
	}

	//완벽 HIT / 찰떡 HIT 둘다 아닌경우
	processNewWord(wordInput.word, UZ, dayBest, weeksBest);
